...
- Sound Lab: guided randomization, Vary/Mutate/Wild mutation strengths, one-click Source/Env/Filter/Sample/FX/Seq/Macro section rolls, recipes, section locks, undo/redo, generated preset draft naming, category/pack/BPM save suggestions, and safety rules.
...
The current test suite includes `EditorLayoutAudit`, which opens the real plugin editor and checks all panels plus FX detail views for visible controls with empty or out-of-bounds layouts, and `PresetSaveAudit`, which verifies nested category saves, normalized preset metadata, and load-by-name behavior.
