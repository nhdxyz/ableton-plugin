...
- SAMPLE: sample load/clear, waveform region editing, phrase markers, transient Detect into eight custom slice regions, slice pads, Slice Keys, reverse, pitch, gain, pan, probability, stutter, choke, nudge, fade, UKG chop helpers, and an adaptive expanded chop focus overlay.
...
- The SAMPLE panel includes a waveform display with chop handles, modulation overlays, selected/custom slice-state badges, slice-lane click selection, first-pass transient Detect slicing into eight custom Slice Keys regions, an adaptive expanded chop focus overlay, and first-pass per-slice nudge/fade actions; remaining work is deeper manual marker editing, per-slice playback modes, slice-to-sequencer flow, full relink, and time-stretch/formant tools.
...
3. Move #77 forward with deeper manual slice-marker editing, per-slice playback modes beyond the first nudge/fade and transient Detect passes, full relink handling beyond the first missing-file stale-audio guard, and slice-to-sequencer lanes before deeper time-stretch/formant work.
