                                 ...
                                 juce::Colour accent)
{
    ...
    auto content = area.reduced(7.0f, 4.0f);
    g.setFont(juce::FontOptions(8.5f, juce::Font::bold));
    g.setColour(juce::Colour(0xff73848a));
    g.drawFittedText(label, content.removeFromTop(10.0f).toNearestInt(), juce::Justification::centredLeft, 1, 0.72f);

    g.setFont(juce::FontOptions(10.5f, juce::Font::bold));
    g.setColour(juce::Colour(0xffdce7e4));
    g.drawFittedText(value, content.toNearestInt(), juce::Justification::centredLeft, 1, 0.7f);
}
...
void PresetSaveSummary::paint(juce::Graphics& g)
{
    ...
    auto content = bounds.reduced(11.0f, 9.0f);
    auto header = content.removeFromTop(18.0f);
    g.setFont(juce::FontOptions(9.0f, juce::Font::bold));
    g.setColour(juce::Colour(0xff73848a));
    g.drawFittedText("SAVE TARGET", header.removeFromLeft(header.getWidth() * 0.45f).toNearestInt(), juce::Justification::centredLeft, 1);
    ...
    g.drawFittedText(state.name, title.toNearestInt(), juce::Justification::centredLeft, 1, 0.68f);

    auto path = content.removeFromTop(18.0f);
    ...
    g.drawFittedText("User Presets / " + state.category + " / " + legalName + ".natevstpreset",
                     path.toNearestInt(),
    ...
    auto pills = content.removeFromTop(38.0f);
    const auto gap = 5.0f;
    ...
    if (content.getHeight() > 18.0f)
    {
        ...
        g.drawFittedText("AUTHOR  " + state.author + "     NOTES  " + juce::String(state.notesCharacters),
                         footer.reduced(7.0f, 3.0f).toNearestInt(),
                         juce::Justification::centredLeft,
                         1,
                         0.62f);
    }
