...
- HOME preset access is now a compact Session view with selected-preset metadata, filter/library counts, favorite/rating/source state, compare readiness, active Random Lab candidate status, and recall/audition controls. The deeper save workflow lives in LIBRARY.
- LIBRARY now uses Find, Browser, Save Patch, and Inspect work areas, plus a compact crate-map summary and dedicated preset/save-target summary components with library stats, visible/total/favorite/rated/factory/generated/macro-rich/style counts, selected-preset metadata, macro shape, notes, destination trail, metadata readiness, generated/overwrite status, and clearer rating placement.
...
- Output visualization: HOME now includes final-output oscilloscope, spectrum, and stereo/correlation displays fed from safe UI-side telemetry. Future work should add freeze/hold, zoomed oscilloscope/spectrum modes, stereo correlation overlay options, and optional FFT detail in an expanded analyzer page.
...
- Session: selected preset, browser filter count, favorite/rating/source state, compare state, random-candidate status, and recall/audition controls.

Current HOME implementation now keeps only the fast patch-building controls visible: Sub/Cutoff/Drive/Output, macros, randomization, signal-flow status, session state, and preset recall/audition. Oscillator waveform selection, filter mode, mono/unison/glide, resonance, filter envelope, noise level, full save metadata, and full envelope editing stay in focused panels.
...
7. Smart mutation workflow: mutation strengths, section-scoped mutation, visible undo/redo history, variation comparison, and save-as-preset handoff.
   ...
   First save-target visualization pass implemented a `PresetSaveSummary` card in LIBRARY with path, metadata, generated/new/existing/overwrite status, notes length, and browser-rating separation. The current pass redesigns it with a destination trail and readiness strip so saving feels more like a preset-vault workflow than a plain form.
...
- HOME feels less like a control dump. Current pass limits HOME to Perform, Macros, Random Lab, Signal Flow, and Session while keeping deeper controls on focused panels.
