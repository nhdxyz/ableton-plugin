...
The current test suite includes `EditorLayoutAudit`, which opens the real plugin editor and checks all panels, every Random Lab page, and FX detail views at default, wide, and maximum editor sizes for visible controls with empty or out-of-bounds layouts, `PresetSaveAudit`, which verifies nested category saves, normalized preset metadata, macro-value preview scanning, and load-by-name behavior, and `RandomCandidateAudit`, which verifies generated-candidate capture, useful-only validation, auto-retry/fallback reporting, compare/diff summaries, non-destructive cue preview, save-from-slot, favorite/rating persistence, recall, and snapshot promotion.
