...
#include "UI/PresetLibraryMetadata.h"
#include "UI/PresetNoteTemplates.h"
#include "UI/RandomRecipeMetadata.h"
...
constexpr auto editorHeaderHeight = 56;
...
namespace PresetLibraryMetadata = UI::PresetLibraryMetadata;
using PresetAuditionRole = PresetLibraryMetadata::PresetAuditionRole;
      ...
      pianoKeyboard(processorToUse.getMidiKeyboardState(), juce::MidiKeyboardComponent::horizontalKeyboard)
{
    ...
    preferenceOptions.applicationName = "Nate VST";
    preferenceOptions.filenameSuffix = "settings";
    preferenceOptions.folderName = "NateVST";
    preferenceOptions.osxLibrarySubFolder = "Application Support";
    preferenceOptions.storageFormat = juce::PropertiesFile::storeAsXML;
    preferenceOptions.millisecondsBeforeSaving = 500;
    editorPreferences = std::make_unique<juce::PropertiesFile>(preferenceOptions);
    loadEditorPreferences();
    lookAndFeel.setTheme(UI::themeFor(selectedTheme));

    setSize(juce::jlimit(editorMinWidth, editorMaxWidth, editorDefaultWidth * uiScalePercent / 100),
            juce::jlimit(editorMinHeight, editorMaxHeight, editorDefaultHeight * uiScalePercent / 100));
    ...
    homeSoundStage.onOscillatorSelected = [this] (bool osc2)
    {
        setSourceFrameActionTarget(osc2);
        updateHomeSoundStage();
    };
    homeSoundStage.onPositionEditStart = [this] (bool osc2)
    {
        captureGlobalEdit(osc2 ? "Edit Osc 2 wavetable position" : "Edit Osc 1 wavetable position");
    };
    homeSoundStage.onPositionChange = [this] (bool osc2, float position)
    {
        const auto waveID = osc2 ? juce::String(Parameters::ID::osc2Wave) : juce::String(Parameters::ID::oscWave);
        const auto positionID = osc2 ? juce::String(Parameters::ID::osc2WavetablePosition)
                                     : juce::String(Parameters::ID::oscWavetablePosition);
        setPlainParameterValue(waveID, 4.0f);
        setPlainParameterValue(positionID, position);
        updateSelectedControlInspector(osc2 ? "WT 2" : "WT 1", positionID, position);
        updateSegmentedSelectors();
        updateWavetableDisplay();
        updateHomeSoundStage();
    };
    homeSoundStage.onOpenEditor = [this] (bool osc2)
    {
        setSourceFrameActionTarget(osc2);
        setActivePanel(Panel::synth);
    };
    addAndMakeVisible(homeSoundStage);
    addAndMakeVisible(homeSignalFlowDisplay);
    addAndMakeVisible(homeSessionDisplay);
    addAndMakeVisible(lowEndAssistant);

    headerViewButton.setTooltip("Choose theme, interface scale, animation, analyzer, and keyboard visibility");
    headerViewButton.onClick = [this] { showViewMenu(); };
    addAndMakeVisible(headerViewButton);

    headerFavoriteButton.setClickingTogglesState(true);
    headerFavoriteButton.setTooltip("Favorite or unfavorite the selected preset");
    headerFavoriteButton.onClick = [this]
    {
        toggleFavoritePreset();
        updateFavoritePresetButton();
    };
    addAndMakeVisible(headerFavoriteButton);

    homeAnalyzerButton.setClickingTogglesState(true);
    homeAnalyzerButton.setToggleState(homeAnalyzerExpanded, juce::dontSendNotification);
    homeAnalyzerButton.setTooltip("Show or hide detailed output analysis on HOME");
    homeAnalyzerButton.onClick = [this]
    {
        homeAnalyzerExpanded = homeAnalyzerButton.getToggleState();
        saveEditorPreferences();
        resized();
    };
    addAndMakeVisible(homeAnalyzerButton);

    homeStageModeButton.setClickingTogglesState(true);
    homeStageModeButton.setToggleState(homeStagePerspective, juce::dontSendNotification);
    homeStageModeButton.setTooltip("Switch the HOME sound stage between perspective and precise flat views");
    homeStageModeButton.onClick = [this]
    {
        homeStagePerspective = homeStageModeButton.getToggleState();
        homeStageModeButton.setButtonText(homeStagePerspective ? "3D" : "2D");
        saveEditorPreferences();
        updateHomeSoundStage();
    };
    homeStageModeButton.setButtonText(homeStagePerspective ? "3D" : "2D");
    addAndMakeVisible(homeStageModeButton);

    pianoKeyboard.setAvailableRange(UI::PianoKeyboardLayout::visualLowestNote, UI::PianoKeyboardLayout::visualHighestNote);
    pianoKeyboard.setLowestVisibleKey(UI::PianoKeyboardLayout::visualLowestNote);
    pianoKeyboard.setOctaveForMiddleC(UI::PianoKeyboardLayout::middleCOctave);
    ...
    pianoKeyboard.setKeyWidth(UI::PianoKeyboardLayout::minimumWhiteKeyWidth);
    ...
    randomRecipeInfoLabel.setText(UI::RandomRecipeMetadata::infoText("Deep House Bass"), juce::dontSendNotification);
    ...
    sourceFrameTargetOsc1Button.setClickingTogglesState(false);
    sourceFrameTargetOsc1Button.onClick = [this] { setSourceFrameActionTarget(false); };
    addAndMakeVisible(sourceFrameTargetOsc1Button);

    sourceFrameTargetOsc2Button.setTooltip("Target Osc 2 for direct wavetable frame actions");
    sourceFrameTargetOsc2Button.setClickingTogglesState(false);
    ...
    sourceFrameEvolveButton.setTooltip("Generate an 8-frame wavetable stack from the selected active frame");
    sourceFrameEvolveButton.onClick = [this]
    {
        evolveCustomWaveFrameStackFromActiveFrame(sourceFrameActionTargetIsOsc2());
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameEvolveButton);
    ...
    {
        copyCurrentCustomWaveFrame(sourceFrameActionTargetIsOsc2());
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameCopyButton);

    sourceFramePasteButton.setTooltip("Paste the copied frame to the active frame in the selected oscillator stack");
    sourceFramePasteButton.onClick = [this]
    {
        pasteCurrentCustomWaveFrame(sourceFrameActionTargetIsOsc2());
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFramePasteButton);

    sourceFrameFillButton.setTooltip("Fill the selected oscillator stack from the active frame");
    sourceFrameFillButton.onClick = [this]
    {
        duplicateCurrentCustomWaveFrameAcrossStack(sourceFrameActionTargetIsOsc2());
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameFillButton);

    sourceFrameInterpolateButton.setTooltip("Interpolate all selected oscillator frames between frame 1 and frame 8");
    sourceFrameInterpolateButton.onClick = [this]
    {
        interpolateCustomWaveFrameEndpoints(sourceFrameActionTargetIsOsc2());
        updateSourceFrameActionButtons();
    };
    ...
    sourceFrameDuplicateSlotButton.setTooltip("Duplicate the active wavetable frame into the next slot");
    sourceFrameDuplicateSlotButton.onClick = [this]
    {
        applyCustomWaveFrameSlotEdit(sourceFrameActionTargetIsOsc2(), WavetableFrameSlotEdit::duplicateAfter);
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameDuplicateSlotButton);

    sourceFrameDeleteSlotButton.setTooltip("Delete the active wavetable frame slot and close the gap");
    sourceFrameDeleteSlotButton.onClick = [this]
    {
        applyCustomWaveFrameSlotEdit(sourceFrameActionTargetIsOsc2(), WavetableFrameSlotEdit::deleteFrame);
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameDeleteSlotButton);

    sourceFrameMoveLeftButton.setTooltip("Move the active wavetable frame one slot left");
    sourceFrameMoveLeftButton.onClick = [this]
    {
        applyCustomWaveFrameSlotEdit(sourceFrameActionTargetIsOsc2(), WavetableFrameSlotEdit::moveLeft);
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameMoveLeftButton);

    sourceFrameMoveRightButton.setTooltip("Move the active wavetable frame one slot right");
    sourceFrameMoveRightButton.onClick = [this]
    {
        applyCustomWaveFrameSlotEdit(sourceFrameActionTargetIsOsc2(), WavetableFrameSlotEdit::moveRight);
        updateSourceFrameActionButtons();
    };
    addAndMakeVisible(sourceFrameMoveRightButton);
    ...
    {
        setSourceFrameActionTarget(false);
    ...
    {
        setSourceFrameActionTarget(true);
    ...
    wavetableDisplay.onFrameAction = [this] (int oscillator, size_t frameIndex, UI::WavetableDisplay::FrameAction action)
    {
        auto mappedAction = WavetableFrameCardAction::copy;
        switch (action)
        {
            case UI::WavetableDisplay::FrameAction::copy:
                mappedAction = WavetableFrameCardAction::copy;
                break;
            case UI::WavetableDisplay::FrameAction::paste:
                mappedAction = WavetableFrameCardAction::paste;
                break;
            case UI::WavetableDisplay::FrameAction::storeMorph:
                mappedAction = WavetableFrameCardAction::storeMorph;
                break;
        }

        handleWavetableFrameCardAction(oscillator == 2, frameIndex, mappedAction);
    };
    ...
    sourceLabFrameStrip.onFrameAction = [this] (bool targetOsc2, size_t frameIndex, UI::WavetableFrameStrip::FrameAction action)
    {
        ...
        handleWavetableFrameCardAction(targetOsc2, frameIndex, mappedAction);
    };
    ...
    waveEditorFocusButton.onClick = [this]
    {
        ...
        const auto targetOsc2 = wavetableTargetIsOsc2();

        if (! osc1IsCustom && ! osc2IsCustom)
        {
            captureGlobalEdit("Open wave editor");
            setPlainParameterValue(targetOsc2 ? Parameters::ID::osc2Wave : Parameters::ID::oscWave, 7.0f);
            (targetOsc2 ? osc2WaveBox : waveformBox).setSelectedItemIndex(7, juce::dontSendNotification);
            updateSegmentedSelectors();
        }
    ...
    wavetableToolBox.addItem("Duplicate Frame Slot", 74);
    wavetableToolBox.addItem("Delete Frame Slot", 75);
    wavetableToolBox.addItem("Move Frame Left", 76);
    wavetableToolBox.addItem("Move Frame Right", 77);
...
void NateVSTAudioProcessorEditor::loadEditorPreferences()
{
    if (editorPreferences == nullptr)
        return;
    ...
    uiScalePercent = storedScale <= 90 ? 90 : (storedScale >= 110 ? 110 : 100);
}

void NateVSTAudioProcessorEditor::saveEditorPreferences()
{
    if (editorPreferences == nullptr)
        return;
    ...
    editorPreferences->saveIfNeeded();
}

void NateVSTAudioProcessorEditor::applySelectedTheme()
{
    lookAndFeel.setTheme(UI::themeFor(selectedTheme));
    applyThemeColours();
    resized();
...
int NateVSTAudioProcessorEditor::currentKeyboardHeight() const noexcept
{
    return keyboardCollapsed ? 0 : pianoKeyboardHeight;
}

void NateVSTAudioProcessorEditor::showViewMenu()
{
    juce::PopupMenu menu;
    menu.addSectionHeader("Theme");
    menu.addItem(101, "Dark Club", true, selectedTheme == UI::ThemeId::darkClub);
    menu.addItem(102, "Analyzer", true, selectedTheme == UI::ThemeId::analyzer);
    menu.addItem(103, "Warm Studio", true, selectedTheme == UI::ThemeId::warmStudio);
    menu.addSeparator();
    menu.addSectionHeader("Interface Scale");
    menu.addItem(201, "90%", true, uiScalePercent == 90);
    menu.addItem(202, "100%", true, uiScalePercent == 100);
    menu.addItem(203, "110%", true, uiScalePercent == 110);
    menu.addSeparator();
    menu.addItem(301, "Animate Visuals", true, animationsEnabled);
    menu.addItem(302, "HOME Analysis", true, homeAnalyzerExpanded);
    menu.addItem(303, "Show Keyboard", true, ! keyboardCollapsed);
    menu.addSeparator();
    menu.addItem(400, "Reset View");

    auto options = juce::PopupMenu::Options().withTargetComponent(&headerViewButton);
    menu.showMenuAsync(options, [safeThis = juce::Component::SafePointer<NateVSTAudioProcessorEditor>(this)] (int result)
    {
        ...
        {
            editor.selectedTheme = static_cast<UI::ThemeId>(result - 101);
            editor.applySelectedTheme();
        }
        else if (result >= 201 && result <= 203)
        {
            editor.uiScalePercent = result == 201 ? 90 : (result == 203 ? 110 : 100);
            editor.setSize(juce::jlimit(editorMinWidth, editorMaxWidth, editorDefaultWidth * editor.uiScalePercent / 100),
                           juce::jlimit(editorMinHeight, editorMaxHeight, editorDefaultHeight * editor.uiScalePercent / 100));
        }
        else if (result == 301)
        {
            editor.animationsEnabled = ! editor.animationsEnabled;
            editor.updateHomeSoundStage();
        }
        else if (result == 302)
        {
            editor.homeAnalyzerExpanded = ! editor.homeAnalyzerExpanded;
            editor.homeAnalyzerButton.setToggleState(editor.homeAnalyzerExpanded, juce::dontSendNotification);
            editor.resized();
        }
        else if (result == 303)
        {
            editor.keyboardCollapsed = ! editor.keyboardCollapsed;
            editor.resized();
            editor.repaint();
        }
        else if (result == 400)
...
void NateVSTAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    const auto topArea = bounds.removeFromTop(editorHeaderHeight);
    const auto keyboardArea = bounds.removeFromBottom(currentKeyboardHeight());
    ...
    if (activePanel == Panel::home)
    {
        ...
        const auto dockHeight = juce::jlimit(116, 154, homeContent.getHeight() / 4);
        auto bottomDock = homeContent.removeFromBottom(dockHeight);
        homeContent.removeFromBottom(8);
        auto sourceArea = homeContent.removeFromLeft(juce::jlimit(244, 286, homeContent.getWidth() / 5)).reduced(5);
        homeContent.removeFromLeft(8);
        auto performanceArea = homeContent.removeFromRight(juce::jlimit(270, 326, homeContent.getWidth() / 3)).reduced(5);
        homeContent.removeFromRight(8);
        auto stageArea = homeContent.reduced(5);
        auto randomArea = homeAnalyzerExpanded
            ? bottomDock.removeFromLeft(juce::jlimit(350, 438, bottomDock.getWidth() / 3)).reduced(5)
            : bottomDock.reduced(5);
        if (homeAnalyzerExpanded)
            bottomDock.removeFromLeft(8);
        auto analysisArea = bottomDock.reduced(5);

        for (auto area : { sourceArea, stageArea, performanceArea, randomArea })
        {
        ...
        {
            g.setColour(theme.panelAlt);
            g.fillRoundedRectangle(analysisArea.toFloat(), 6.0f);
            g.setColour(theme.outline);
            g.drawRoundedRectangle(analysisArea.toFloat(), 6.0f, 1.0f);
        }
    }
...
{
    auto bounds = getLocalBounds().reduced(16);
    auto header = bounds.removeFromTop(editorHeaderHeight);
    auto headerTop = header.removeFromTop(30);
    outputMeter.setBounds(headerTop.removeFromRight(156).reduced(6, 5));
    headerViewButton.setBounds(headerTop.removeFromRight(60).reduced(3, 5));
    headerFavoriteButton.setBounds(headerTop.removeFromRight(48).reduced(3, 5));
    titleLabel.setBounds(headerTop.removeFromLeft(126).reduced(8, 0));
    previousPresetButton.setBounds(headerTop.removeFromLeft(36).reduced(3, 5));
    presetBox.setBounds(headerTop.removeFromLeft(juce::jlimit(210, 320, headerTop.getWidth() / 2)).reduced(3, 5));
    nextPresetButton.setBounds(headerTop.removeFromLeft(36).reduced(3, 5));
    panelTabBar.setBounds(header.reduced(6, 2));

    auto keyboardArea = bounds.removeFromBottom(currentKeyboardHeight());
    const auto showKeyboard = ! keyboardArea.isEmpty();
    for (auto* component : std::initializer_list<juce::Component*> {
             &pianoKeyboard, &keyboardOctaveDownButton, &keyboardOctaveUpButton, &keyboardHomeButton,
             &keyboardPanicButton, &keyboardRangeLabel, &keyboardPerformanceLabel })
        component->setVisible(showKeyboard);
    ...
    headerFavoriteButton.setVisible(true);
    headerViewButton.setVisible(true);
    outputMeter.setVisible(true);
    panelTabBar.setVisible(true);
    ...
    switch (activePanel)
    {
        case Panel::home:
        {
            ...
            homeSoundStage.setVisible(true);
            homeSignalFlowDisplay.setVisible(homeAnalyzerExpanded);
            outputOscilloscopeDisplay.setVisible(homeAnalyzerExpanded);
            outputSpectrumDisplay.setVisible(homeAnalyzerExpanded);
            stereoFieldDisplay.setVisible(homeAnalyzerExpanded);
            ...
            const auto dockHeight = juce::jlimit(116, 154, dashboard.getHeight() / 4);
            auto bottomDock = dashboard.removeFromBottom(dockHeight);
            dashboard.removeFromBottom(8);
            auto sourceArea = dashboard.removeFromLeft(juce::jlimit(244, 286, dashboard.getWidth() / 5)).reduced(14, 10);
            dashboard.removeFromLeft(8);
            auto macroArea = dashboard.removeFromRight(juce::jlimit(270, 326, dashboard.getWidth() / 3)).reduced(14, 10);
            dashboard.removeFromRight(8);
            auto stageArea = dashboard.reduced(10, 8);
            auto labArea = homeAnalyzerExpanded
                ? bottomDock.removeFromLeft(juce::jlimit(350, 438, bottomDock.getWidth() / 3)).reduced(14, 10)
                : bottomDock.reduced(14, 10);
            if (homeAnalyzerExpanded)
                bottomDock.removeFromLeft(8);
            auto analysisArea = bottomDock.reduced(10, 8);

            auto sourceHeader = sourceArea.removeFromTop(24);
            homeEngineLabel.setBounds(sourceHeader);
            houseLayerRackDisplay.setBounds(sourceArea.removeFromTop(juce::jmax(150, sourceArea.getHeight() - 154)).reduced(2, 4));
            ...
            layoutKnobRow(sourceArea.removeFromTop(82).withTrimmedTop(4), { &subLevelSlider, &cutoffSlider, &driveSlider, &outputSlider });
            lowEndAssistant.setBounds(sourceArea.reduced(2, 4));

            auto stageHeader = stageArea.removeFromTop(26);
            homeAnalyzerButton.setBounds(stageHeader.removeFromRight(76).reduced(2, 2));
            homeStageModeButton.setBounds(stageHeader.removeFromRight(48).reduced(2, 2));
            homeSoundStage.setBounds(stageArea.reduced(2, 0));
            ...
            if (homeAnalyzerExpanded)
            {
                homeSignalFlowDisplay.setBounds(analysisArea.removeFromTop(42).reduced(2, 0));
                auto liveRow = analysisArea.withTrimmedTop(5);
                const auto scopeWidth = juce::jlimit(150, 230, liveRow.getWidth() / 3);
                const auto stereoWidth = juce::jlimit(92, 132, liveRow.getWidth() / 5);
                outputOscilloscopeDisplay.setBounds(liveRow.removeFromLeft(scopeWidth).reduced(3, 1));
                stereoFieldDisplay.setBounds(liveRow.removeFromRight(stereoWidth).reduced(3, 1));
                outputSpectrumDisplay.setBounds(liveRow.reduced(3, 1));
            }
            break;
        }
    ...
    auto persistentHeader = getLocalBounds().reduced(16).removeFromTop(editorHeaderHeight);
    auto persistentHeaderTop = persistentHeader.removeFromTop(30);
    outputMeter.setBounds(persistentHeaderTop.removeFromRight(156).reduced(6, 5));
    headerViewButton.setBounds(persistentHeaderTop.removeFromRight(60).reduced(3, 5));
    headerFavoriteButton.setBounds(persistentHeaderTop.removeFromRight(48).reduced(3, 5));
    titleLabel.setBounds(persistentHeaderTop.removeFromLeft(126).reduced(8, 0));
    previousPresetButton.setBounds(persistentHeaderTop.removeFromLeft(36).reduced(3, 5));
    presetBox.setBounds(persistentHeaderTop.removeFromLeft(juce::jlimit(210, 320, persistentHeaderTop.getWidth() / 2)).reduced(3, 5));
    nextPresetButton.setBounds(persistentHeaderTop.removeFromLeft(36).reduced(3, 5));
    panelTabBar.setBounds(persistentHeader.reduced(6, 2));

    for (auto* component : std::initializer_list<juce::Component*> {
             &titleLabel, &previousPresetButton, &presetBox, &nextPresetButton,
             &headerFavoriteButton, &headerViewButton, &outputMeter, &panelTabBar })
    {
...
juce::StringArray NateVSTAudioProcessorEditor::runLayoutAudit()
{
    ...
    const auto originalKeyboardCollapsed = keyboardCollapsed;
    const auto originalAnalyzerExpanded = homeAnalyzerExpanded;
    juce::StringArray issues;

    keyboardCollapsed = false;
    homeAnalyzerExpanded = true;
    homeAnalyzerButton.setToggleState(true, juce::dontSendNotification);
    ...
    auto auditComputerKeyboardRange = [this, &issues]
    {
        auto expectBase = [&issues, this] (const juce::String& state, int expectedNote)
        {
            ...
            if (baseNote != expectedNote)
            {
                issues.add("Keyboard " + state + ": laptop A maps to MIDI "
                           ...
                           + " (" + UI::PianoKeyboardLayout::noteName(baseNote) + ") instead of MIDI "
                           + juce::String(expectedNote)
                           + " (" + UI::PianoKeyboardLayout::noteName(expectedNote) + ")");
            }
            ...
            if (pianoKeyboard.getLowestVisibleKey() != UI::PianoKeyboardLayout::visualLowestNote)
            {
                issues.add("Keyboard " + state + ": visible piano strip starts at MIDI "
                           + juce::String(pianoKeyboard.getLowestVisibleKey())
                           + " (" + UI::PianoKeyboardLayout::noteName(pianoKeyboard.getLowestVisibleKey())
                           + ") instead of fixed MIDI "
                           + juce::String(UI::PianoKeyboardLayout::visualLowestNote)
                           + " (" + UI::PianoKeyboardLayout::noteName(UI::PianoKeyboardLayout::visualLowestNote) + ")");
            }

            const auto expectedKeyWidth = UI::PianoKeyboardLayout::responsiveKeyWidthForBounds(pianoKeyboard.getBounds());
        ...
        pinPianoKeyboardVisualRange();
        updateKeyboardRangeLabel();
        expectBase("home", UI::PianoKeyboardLayout::initialLowestNote);
        if (pianoKeyboard.getOctaveForMiddleC() != UI::PianoKeyboardLayout::middleCOctave)
            issues.add("Keyboard home: piano strip C labels use middle-C octave "
                       ...
                       + juce::String(UI::PianoKeyboardLayout::middleCOctave));
        ...
        expectBase("Oct+", UI::PianoKeyboardLayout::maxLowestVisibleNote);
        if (keyboardRangeLabel.getText() != "A:C4\n;:E5")
            issues.add("Keyboard Oct+: range label should read A:C4 to ;:E5, got "
                       + keyboardRangeLabel.getText().quoted());

        shiftKeyboardOctave(12);
        expectBase("Oct+ clamp", UI::PianoKeyboardLayout::maxLowestVisibleNote);
        ...
        shiftKeyboardOctave(-12);
        expectBase("Oct- floor", UI::PianoKeyboardLayout::minLowestVisibleNote);
        if (keyboardRangeLabel.getText() != "A:C1\n;:E2")
            issues.add("Keyboard Oct- floor: range label should read A:C1 to ;:E2, got "
                       + keyboardRangeLabel.getText().quoted());

        shiftKeyboardOctave(-12);
        expectBase("Oct- clamp", UI::PianoKeyboardLayout::minLowestVisibleNote);
    ...
    auto auditCurrentLayout = [this, &issues] (const juce::String& panelName)
    {
        ...
        {
            if (! homeSoundStage.isVisible())
            {
                issues.add(panelName + ": interactive HOME sound stage is not visible");
            }
            else
            {
                const auto metrics = homeSoundStage.getLayoutMetricsForAudit();
                if (! metrics.readable)
                {
                    issues.add(panelName + ": HOME sound stage is too small "
                               + juce::String(metrics.stageWidth, 1) + "x"
                               + juce::String(metrics.stageHeight, 1));
                }
            }
        }
        ...
        if (panelName.contains("SYNTH"))
        {
            ...
            if (panelName.contains("Source Focus"))
            {
                ...
                for (const auto* button : {
                         ...
                         static_cast<const juce::Component*>(&sourceFrameEvolveButton),
                         ...
                         static_cast<const juce::Component*>(&sourceFrameDuplicateSlotButton),
                         static_cast<const juce::Component*>(&sourceFrameDeleteSlotButton),
                         static_cast<const juce::Component*>(&sourceFrameMoveLeftButton),
                         static_cast<const juce::Component*>(&sourceFrameMoveRightButton) })
                {
                    if (! button->isVisible())
                    {
                        issues.add(panelName + ": source lab frame action control is not visible");
                        continue;
                    }

                    const auto buttonBounds = button->getBounds();
                    if (buttonBounds.getWidth() < 38 || buttonBounds.getHeight() < 20)
                    {
                        issues.add(panelName + ": source lab frame action control is too small "
        ...
        const auto expectedKeyboardKeyWidth = UI::PianoKeyboardLayout::responsiveKeyWidthForBounds(pianoKeyboard.getBounds());
        ...
        const auto keyboardPaintCoverage = static_cast<float>(UI::PianoKeyboardLayout::whiteKeyCountInRange(UI::PianoKeyboardLayout::visualLowestNote,
                                                                                  UI::PianoKeyboardLayout::visualHighestNote))
        ...
        const auto defaultTypingBaseNote = UI::PianoKeyboardLayout::typingBaseNoteForLowestNote(UI::PianoKeyboardLayout::initialLowestNote);
        if (defaultTypingBaseNote != UI::PianoKeyboardLayout::initialLowestNote)
        {
            issues.add(panelName + ": default laptop keyboard maps A to MIDI "
                       + juce::String(defaultTypingBaseNote)
                       + " instead of MIDI "
                       + juce::String(UI::PianoKeyboardLayout::initialLowestNote)
                       + "/" + UI::PianoKeyboardLayout::noteName(UI::PianoKeyboardLayout::initialLowestNote)
    ...
    keyboardCollapsed = originalKeyboardCollapsed;
    homeAnalyzerExpanded = originalAnalyzerExpanded;
    homeAnalyzerButton.setToggleState(homeAnalyzerExpanded, juce::dontSendNotification);
    updateKeyboardRangeLabel();
    pinPianoKeyboardVisualRange();
    updatePanelVisibility();
...
void NateVSTAudioProcessorEditor::shiftKeyboardOctave(int semitones)
{
    ...
    const auto nextBaseNote = UI::PianoKeyboardLayout::clampLowestVisibleNote(currentBaseNote + semitones);
...
{
    const auto mappedBaseNote = UI::PianoKeyboardLayout::clampLowestVisibleNote(keyboardTypingBaseNote);
    ...
    const auto noteName = UI::PianoKeyboardLayout::noteName(mappedBaseNote);
    const auto mappedTopNote = juce::jlimit(0,
                                           UI::PianoKeyboardLayout::highestNote,
                                           mappedBaseNote + UI::PianoKeyboardLayout::typingKeySpanSemitones);
    const auto mappedTopName = UI::PianoKeyboardLayout::noteName(mappedTopNote);
    ...
    keyboardOctaveDownButton.setEnabled(mappedBaseNote > UI::PianoKeyboardLayout::minLowestVisibleNote);
...
int NateVSTAudioProcessorEditor::computerKeyboardBaseNote() const noexcept
{
    return juce::jlimit(UI::PianoKeyboardLayout::minLowestVisibleNote,
                        UI::PianoKeyboardLayout::maxLowestVisibleNote,
...
{
    auto bounds = getLocalBounds().reduced(16);
    bounds.removeFromTop(editorHeaderHeight);
    bounds.removeFromBottom(currentKeyboardHeight());
    bounds.removeFromBottom(10);
...
void NateVSTAudioProcessorEditor::layoutFocusOverlay()
{
    ...
    sourceFrameEvolveButton.setVisible(showSourceLayerEditor);
    ...
    sourceFrameDuplicateSlotButton.setVisible(showSourceLayerEditor);
    sourceFrameDeleteSlotButton.setVisible(showSourceLayerEditor);
    ...
    else if (showSourceLayerEditor)
    {
        ...
        auto frameActionArea = content.removeFromBottom(juce::jlimit(124, 140, content.getHeight() / 4)).withTrimmedBottom(2);
        const auto frameActionRowHeight = juce::jmax(30, frameActionArea.getHeight() / 4);
        auto frameActionRow = frameActionArea.removeFromTop(frameActionRowHeight);
        auto targetArea = frameActionRow.removeFromLeft(98);
        sourceFrameTargetOsc1Button.setBounds(targetArea.removeFromLeft(49).reduced(3, 4));
        sourceFrameTargetOsc2Button.setBounds(targetArea.reduced(3, 4));
        frameActionRow.removeFromLeft(8);
        sourceFramePreviousButton.setBounds(frameActionRow.removeFromLeft(62).reduced(3, 4));
        sourceFrameNextButton.setBounds(frameActionRow.removeFromLeft(62).reduced(3, 4));
        sourceFrameEvolveButton.setBounds(frameActionRow.removeFromLeft(92).reduced(3, 4));
        auto editActionRow = frameActionArea.removeFromTop(frameActionRowHeight).withTrimmedTop(1);
        const auto actionWidth = juce::jmax(1, editActionRow.getWidth() / 4);
        sourceFrameCopyButton.setBounds(editActionRow.removeFromLeft(actionWidth).reduced(3, 4));
        sourceFramePasteButton.setBounds(editActionRow.removeFromLeft(actionWidth).reduced(3, 4));
        sourceFrameFillButton.setBounds(editActionRow.removeFromLeft(actionWidth).reduced(3, 4));
        sourceFrameInterpolateButton.setBounds(editActionRow.reduced(3, 4));
        auto stackActionRow = frameActionArea.removeFromTop(frameActionRowHeight).withTrimmedTop(1);
        const auto stackActionWidth = juce::jmax(1, stackActionRow.getWidth() / 5);
        sourceStackReverseButton.setBounds(stackActionRow.removeFromLeft(stackActionWidth).reduced(3, 4));
        sourceStackRotateLeftButton.setBounds(stackActionRow.removeFromLeft(stackActionWidth).reduced(3, 4));
        sourceStackRotateRightButton.setBounds(stackActionRow.removeFromLeft(stackActionWidth).reduced(3, 4));
        sourceStackSmoothButton.setBounds(stackActionRow.removeFromLeft(stackActionWidth).reduced(3, 4));
        sourceStackEmphasiseButton.setBounds(stackActionRow.reduced(3, 4));
        auto slotActionRow = frameActionArea.withTrimmedTop(1);
        const auto slotActionWidth = juce::jmax(1, slotActionRow.getWidth() / 4);
        sourceFrameDuplicateSlotButton.setBounds(slotActionRow.removeFromLeft(slotActionWidth).reduced(3, 4));
        sourceFrameDeleteSlotButton.setBounds(slotActionRow.removeFromLeft(slotActionWidth).reduced(3, 4));
        sourceFrameMoveLeftButton.setBounds(slotActionRow.removeFromLeft(slotActionWidth).reduced(3, 4));
        sourceFrameMoveRightButton.setBounds(slotActionRow.reduced(3, 4));
    ...
    sourceFrameEvolveButton.toFront(false);
    ...
    sourceFrameDuplicateSlotButton.toFront(false);
    sourceFrameDeleteSlotButton.toFront(false);
...
void NateVSTAudioProcessorEditor::updateRandomRecipeInfo()
{
    ...
    const auto text = UI::RandomRecipeMetadata::infoText(recipeName);
...
{
    if (sourceFrameActionTargetExplicit)
        return sourceFrameActionTargetOsc2;
...
void NateVSTAudioProcessorEditor::setSourceFrameActionTarget(bool targetOsc2)
{
...
{
    return wavetableTargetIsOsc2();
}

void NateVSTAudioProcessorEditor::updateSourceFrameActionButtons()
{
    const auto targetOsc2 = sourceFrameActionTargetIsOsc2();
    const auto targetName = juce::String(targetOsc2 ? "O2" : "O1");
    const auto currentFrame = currentCustomWaveFrameIndex(targetOsc2);
    const auto frameNumber = juce::String(static_cast<int>(currentFrame + 1));
    const auto canMoveLeft = currentFrame > 0;
    const auto canMoveRight = currentFrame + 1 < Parameters::customWaveMorphFrameCount;
    ...
    sourceFramePreviousButton.setEnabled(canMoveLeft);
    sourceFrameNextButton.setEnabled(canMoveRight);
    ...
    sourceFrameDuplicateSlotButton.setEnabled(canMoveRight);
    sourceFrameDeleteSlotButton.setEnabled(true);
    sourceFrameMoveLeftButton.setEnabled(canMoveLeft);
    sourceFrameMoveRightButton.setEnabled(canMoveRight);
    ...
    sourceFrameEvolveButton.setTooltip("Generate a full " + targetName + " stack from active frame " + frameNumber);
    ...
    sourceFrameDuplicateSlotButton.setTooltip(canMoveRight
                                                  ? "Duplicate " + targetName + " frame " + frameNumber + " into the next slot"
                                                  : targetName + " frame 8 cannot duplicate forward");
    sourceFrameDeleteSlotButton.setTooltip("Delete " + targetName + " frame " + frameNumber + " and close the frame gap");
    sourceFrameMoveLeftButton.setTooltip(canMoveLeft
                                             ? "Move " + targetName + " frame " + frameNumber + " one slot left"
                                             : targetName + " frame 1 cannot move left");
    sourceFrameMoveRightButton.setTooltip(canMoveRight
                                              ? "Move " + targetName + " frame " + frameNumber + " one slot right"
                                              : targetName + " frame 8 cannot move right");
}
...
void NateVSTAudioProcessorEditor::handleWavetableFrameCardAction(bool targetOsc2,
                                                                 size_t frameIndex,
                                                                 WavetableFrameCardAction action)
{
    const auto safeFrame = juce::jlimit<size_t>(0, Parameters::customWaveMorphFrameCount - 1, frameIndex);
    const auto targetName = juce::String(targetOsc2 ? "O2" : "O1");
    ...
    switch (action)
    {
        case WavetableFrameCardAction::copy:
        ...
        case WavetableFrameCardAction::paste:
        ...
        case WavetableFrameCardAction::storeMorph:
        {
...
void NateVSTAudioProcessorEditor::evolveCustomWaveFrameStackFromActiveFrame(bool targetOsc2)
{
    const auto frameIndex = currentCustomWaveFrameIndex(targetOsc2);
    const auto seed = readCustomWaveFrame(targetOsc2, frameIndex);
    writeCustomWaveFrameSet(targetOsc2,
                            Synth::WavetableFrameRecipes::currentSweep(seed),
                            "Evolve wavetable stack from active frame",
                            "Evolved " + juce::String(targetOsc2 ? "O2" : "O1")
                                + " stack from frame " + juce::String(static_cast<int>(frameIndex + 1)));
    updateSourceFrameActionButtons();
}
...
void NateVSTAudioProcessorEditor::applyCustomWaveFrameSlotEdit(bool targetOsc2, WavetableFrameSlotEdit edit)
{
    setSourceFrameActionTarget(targetOsc2);

    constexpr auto frameCount = Parameters::customWaveMorphFrameCount;
    const auto lastFrame = frameCount - 1;
    const auto activeFrame = currentCustomWaveFrameIndex(targetOsc2);
    const auto targetName = juce::String(targetOsc2 ? "O2" : "O1");
    const auto sourceFrames = readCustomWaveFrameSet(targetOsc2);
    std::array<UI::WavetableDisplay::CustomPointArray, frameCount> editedFrames {};
    auto selectedFrame = activeFrame;
    juce::String editLabel;
    juce::String statusText;

    switch (edit)
    {
        case WavetableFrameSlotEdit::duplicateAfter:
            if (activeFrame >= lastFrame)
            {
                setRandomStatus(targetName + " frame 8 cannot duplicate forward");
                updateSourceFrameActionButtons();
                returnKeyboardFocusToPiano();
                return;
            }

            editedFrames = Synth::WavetableFrameRecipes::duplicateFrameSlot(sourceFrames, activeFrame);
            selectedFrame = activeFrame + 1;
            editLabel = "Duplicate wavetable frame slot";
            statusText = "Duplicated " + targetName + " frame "
                + juce::String(static_cast<int>(activeFrame + 1))
                + " into slot " + juce::String(static_cast<int>(selectedFrame + 1));
            break;

        case WavetableFrameSlotEdit::deleteFrame:
            editedFrames = Synth::WavetableFrameRecipes::deleteFrameSlot(sourceFrames, activeFrame);
            selectedFrame = activeFrame >= lastFrame ? lastFrame - 1 : activeFrame;
            editLabel = "Delete wavetable frame slot";
            statusText = "Deleted " + targetName + " frame "
                + juce::String(static_cast<int>(activeFrame + 1))
                + " and closed the stack gap";
            break;

        case WavetableFrameSlotEdit::moveLeft:
            if (activeFrame == 0)
            {
                setRandomStatus(targetName + " frame 1 cannot move left");
                updateSourceFrameActionButtons();
                returnKeyboardFocusToPiano();
                return;
            }
            ...
            selectedFrame = activeFrame - 1;
            editedFrames = Synth::WavetableFrameRecipes::moveFrameSlot(sourceFrames, activeFrame, selectedFrame);
            editLabel = "Move wavetable frame left";
            statusText = "Moved " + targetName + " frame "
                + juce::String(static_cast<int>(activeFrame + 1))
                + " left to slot " + juce::String(static_cast<int>(selectedFrame + 1));
            break;

        case WavetableFrameSlotEdit::moveRight:
            if (activeFrame >= lastFrame)
            {
                setRandomStatus(targetName + " frame 8 cannot move right");
                updateSourceFrameActionButtons();
                returnKeyboardFocusToPiano();
                return;
            }
            ...
            selectedFrame = activeFrame + 1;
            editedFrames = Synth::WavetableFrameRecipes::moveFrameSlot(sourceFrames, activeFrame, selectedFrame);
            editLabel = "Move wavetable frame right";
            statusText = "Moved " + targetName + " frame "
                + juce::String(static_cast<int>(activeFrame + 1))
                + " right to slot " + juce::String(static_cast<int>(selectedFrame + 1));
            break;
    }

    writeCustomWaveFrameSet(targetOsc2, editedFrames, editLabel, statusText);

    const auto parameterID = targetOsc2 ? juce::String(Parameters::ID::osc2WavetablePosition)
                                        : juce::String(Parameters::ID::oscWavetablePosition);
    const auto position = static_cast<float>(selectedFrame) / static_cast<float>(lastFrame);
    setPlainParameterValue(parameterID, position);
    updateSelectedControlInspector(targetName + " Frame " + juce::String(static_cast<int>(selectedFrame + 1)),
                                   parameterID,
                                   position);
    updateWavetableDisplay();
    updateSourceLabFrameStrip();
...
void NateVSTAudioProcessorEditor::applySelectedWavetableTool()
{
    ...
    switch (selectedId)
    {
        ...
        case 74:
            applyCustomWaveFrameSlotEdit(targetOsc2, WavetableFrameSlotEdit::duplicateAfter);
            changed = false;
            break;

        case 75:
            applyCustomWaveFrameSlotEdit(targetOsc2, WavetableFrameSlotEdit::deleteFrame);
            changed = false;
            break;

        case 76:
            applyCustomWaveFrameSlotEdit(targetOsc2, WavetableFrameSlotEdit::moveLeft);
            changed = false;
            break;

        case 77:
            applyCustomWaveFrameSlotEdit(targetOsc2, WavetableFrameSlotEdit::moveRight);
            changed = false;
            break;
...
void NateVSTAudioProcessorEditor::updateHomeSoundStage()
{
    UI::HomeSoundStage::State state;
    const auto osc1Wave = juce::jlimit(0, 7, juce::roundToInt(readPlainParameterValue(Parameters::ID::oscWave, 1.0f)));
    const auto osc2Wave = juce::jlimit(0, 7, juce::roundToInt(readPlainParameterValue(Parameters::ID::osc2Wave, 1.0f)));
    const auto osc1Level = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::osc1Level, 1.0f));
    const auto osc2Level = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::osc2Level, 0.0f));

    state.oscillators[0].name = "OSC 1";
    state.oscillators[0].waveName = UI::SourceWaveLabels::nameForWave(osc1Wave);
    state.oscillators[0].waveType = osc1Wave;
    state.oscillators[0].level = osc1Level;
    state.oscillators[0].position = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::oscWavetablePosition, 0.0f));
    state.oscillators[0].warp = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::oscWarp, 0.0f));
    state.oscillators[0].active = osc1Level > 0.02f;
    state.oscillators[0].frames = readCustomWaveFrameSet(false);

    state.oscillators[1].name = "OSC 2";
    state.oscillators[1].waveName = UI::SourceWaveLabels::nameForWave(osc2Wave);
    state.oscillators[1].waveType = osc2Wave;
    state.oscillators[1].level = osc2Level;
    state.oscillators[1].position = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::osc2WavetablePosition, 0.0f));
    state.oscillators[1].warp = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::osc2Warp, 0.0f));
    state.oscillators[1].active = osc2Level > 0.02f;
    state.oscillators[1].frames = readCustomWaveFrameSet(true);
    ...
    state.sourceLevels = {
        ...
        juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::subLevel, 0.0f)),
        juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::noiseLevel, 0.0f)),
        sampleEnabled ? juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::sampleMix, 0.0f)) : 0.0f
    };

    for (size_t index = 0; index < state.macros.size(); ++index)
        state.macros[index] = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(macroPerformanceParameterIDs[index], 0.0f));

    const auto cutoffHz = juce::jlimit(20.0f, 20000.0f, readPlainParameterValue(Parameters::ID::filterCutoff, 1000.0f));
    state.cutoff = juce::jlimit(0.0f, 1.0f, std::log(cutoffHz / 20.0f) / std::log(1000.0f));
    state.drive = juce::jlimit(0.0f, 1.0f, readPlainParameterValue(Parameters::ID::driveAmount, 0.0f));
    state.outputPeak = juce::jlimit(0.0f, 1.0f, juce::jmax(displayedPeakLeft, displayedPeakRight));
    state.animationPhase = static_cast<float>(std::fmod(juce::Time::getMillisecondCounterHiRes() * 0.00012, 1.0));
    state.presetName = presetBox.getText().trim().isNotEmpty() ? presetBox.getText().trim() : juce::String("Init");
    state.safetyName = state.outputPeak >= 0.985f ? juce::String("CLIP")
                     : state.outputPeak >= 0.9f ? juce::String("HOT")
                     : state.outputPeak < 0.08f ? juce::String("LOW")
                                               : juce::String("SAFE");
    state.perspective = homeStagePerspective;
    state.animate = animationsEnabled;
    state.osc2Selected = sourceFrameActionTargetIsOsc2();
    homeSoundStage.setState(std::move(state));
}
...
void NateVSTAudioProcessorEditor::updateHomeSessionDisplay()
{
    ...
    for (const auto& preset : library)
    {
        ...
        selectedSearchText = PresetLibraryMetadata::searchText(preset);
                                                    ...
                                                    bool rowIsSelected)
{
    ...
    const auto roleText = PresetLibraryMetadata::inspectorRoleName(PresetLibraryMetadata::inferAuditionRole(&preset));
    ...
    PresetLibraryMetadata::drawPreviewLevelBadge(g, previewArea.removeFromTop(layout.compact ? 13 : 15), preset);
...
void NateVSTAudioProcessorEditor::selectedRowsChanged(int lastRowSelected)
{
    ...
    presetStatusLabel.setText("Selected " + preset.name + " | " + preset.pack + " | "
                                  + preset.key + " | " + UI::PresetLibraryChoices::formatBpm(preset.bpm)
                                  + " | " + PresetLibraryMetadata::previewSummaryText(preset)
                                  + " | " + PresetLibraryMetadata::macroPreviewText(preset),
                              juce::dontSendNotification);
...
void NateVSTAudioProcessorEditor::refreshPresetList()
{
    ...
    auto matchesSearch = [&searchTerms] (const NateVSTAudioProcessor::PresetInfo& preset)
    {
        ...
        const auto searchable = PresetLibraryMetadata::searchText(preset) + " "
            + PresetLibraryMetadata::macroPreviewText(preset) + " Macro Macros Performance";
    ...
    auto matchesSimilarPreset = [&similarReferencePreset, hasSimilarReference] (const NateVSTAudioProcessor::PresetInfo& preset)
    {
        ...
        auto score = 0;
        if (PresetLibraryMetadata::inferAuditionRole(&preset) == PresetLibraryMetadata::inferAuditionRole(&similarReferencePreset))
            score += 3;
    ...
    else
    {
        ...
        for (const auto& preset : library)
        {
            ...
            const auto matchesFilter = filter == "All"
                ...
                || PresetLibraryMetadata::matchesSmartCrate(preset, filter)
    ...
    if (selectedPreset != nullptr)
        statusText += " | " + PresetLibraryMetadata::previewSummaryText(*selectedPreset) + " | " + PresetLibraryMetadata::macroPreviewText(*selectedPreset);
    ...
    presetStatusLabel.setTooltip(selectedPreset != nullptr
                                     ? PresetLibraryMetadata::previewSummaryText(*selectedPreset) + " | "
                                           + PresetLibraryMetadata::macroPreviewText(*selectedPreset) + " | " + selectedPreset->pack + " | " + selectedPreset->key + " | " + UI::PresetLibraryChoices::formatBpm(selectedPreset->bpm)
    ...
    presetBox.setTooltip(selectedPreset != nullptr
                             ? selectedPreset->name + " | " + PresetLibraryMetadata::macroPreviewText(*selectedPreset)
...
void NateVSTAudioProcessorEditor::updatePresetLibrarySummary()
{
    ...
    for (const auto& preset : library)
    {
        ...
        state.selectedRole = PresetLibraryMetadata::inspectorRoleName(PresetLibraryMetadata::inferAuditionRole(&preset));
        state.selectedTraits = PresetLibraryMetadata::inspectorTraits(preset);
        state.selectedCue = PresetLibraryMetadata::inspectorCue(preset);
        state.macroValues = preset.macroValues;
        state.profileValues = PresetLibraryMetadata::inspectorProfile(preset);
        break;
    }
...
void NateVSTAudioProcessorEditor::auditionSelectedPreset()
{
    ...
    const auto roleText = hasAuditionPresetInfo ? PresetLibraryMetadata::inspectorRoleName(PresetLibraryMetadata::inferAuditionRole(&auditionPresetInfo))
                                                : juce::String("Preset");
...
juce::String NateVSTAudioProcessorEditor::startPresetAuditionPhrase(const NateVSTAudioProcessor::PresetInfo* presetInfo, int rootNote)
{
    ...
    const auto role = PresetLibraryMetadata::inferAuditionRole(presetInfo);
    const auto nowMs = juce::Time::getMillisecondCounterHiRes();
    ...
    updatePresetAudition();
    return PresetLibraryMetadata::auditionRoleName(role);
}
...
{
    const auto presetName = presetBox.getText().trim();
    const auto isFavorite = presetName.isNotEmpty() && audioProcessor.isPresetFavorite(presetName);
    headerFavoriteButton.setEnabled(presetName.isNotEmpty());
    headerFavoriteButton.setToggleState(isFavorite, juce::dontSendNotification);
    ...
    for (const auto& preset : audioProcessor.getPresetLibrary())
    {
        ...
        const auto preview = PresetLibraryMetadata::macroPreviewText(preset);
        const auto renderedPreview = PresetLibraryMetadata::previewSummaryText(preset);
