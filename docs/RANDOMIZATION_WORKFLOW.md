...
- `Sample`: sample enable, slice range, reverse, playback mode, sample engine mode, grain size, grain spray, spectral freeze, stutter, pitch, pitch ramp, gain, mix, and loaded sample path.
...
- `Seq`: sequencer enable/rate/root/gate/swing/groove/scale/chord/voicing/strum/accent/octave/probability/random amount, lock destination/depth, and step pattern, including per-step timing, length, and lock values.
...
Recorded sample mangle and UKG chop randomization can now seed `Classic`, `Granular`, `Spectral`, or `Cloud` sample engine behavior. Grain size, spray, and freeze are randomized inside conservative ranges so generated snippets can become texture beds, cloud chops, or frozen vocal/perc fragments without requiring a separate editor pass.
