...
- The Library now has metadata, smart crates, ratings/favorites, rendered preview cache/playback, visible-row preview warming, preview level badges, compact-width browser rows, and 438 factory presets. Missing next: fully background preview generation, waveform preview badges, construction-kit browsing, dependency/relink warnings, pack import/export, and user crate editing.
...
- Library includes recursive user/factory scanning, metadata, ratings, favorites, smart crates, style filters, crate map, selected-preset profile, compare/revert, safe overwrite, 438 repo-managed factory presets, construction-kit style metadata, rendered preview cache/playback, visible-row preview warming, preview level badges, and compact rows that keep sound identity readable at narrow widths.
...
Nate VST now has metadata, smart crates, preview cache/playback, ratings, favorites, crate maps, and 438 curated factory presets. The gap is now preview workflow polish, deeper pack navigation, and construction-kit browsing:
