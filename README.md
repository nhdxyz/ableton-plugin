...
- Library: save with a visual target trail, metadata readiness preview, and same-folder overwrite confirmation, load, role-aware audition phrases, compare/revert recent preset loads, search, sort, smart crates, tag, favorite, rate, an adaptive selected-preset profile card with role/traits/cue/profile meters, macro-value preview, preset notes, and metadata-label categorized `.natevstpreset` files from recursive user and factory preset folders, including House, Tech House, Techno, Minimal, UKG, pack, key, and tempo filters.
