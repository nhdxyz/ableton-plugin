...
Current house/UKG workflow focus: cleaner oscillator sources, internal wavetable movement, Osc Warp source motion, Filter Character/Slope modes for bass and stab color, HOME performance macros and club low-end guidance, MOD destination inspection with per-route bypass/delete, SEQ chord/voicing/strum plus one-finger Memory, host-sync status, and selective groove transforms, waveform-based sample chopping with click guards, slice styles, Slice Keys, and per-slice region/pan/chance/choke memory, section-scoped randomization, and an addable FX rack with throw controls and module presets.
