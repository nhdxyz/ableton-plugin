...
- Sampler depth: the waveform now shows eight saved slice regions with selected/custom state, performance badges, first-pass transient Detect creation, and first-pass nudge/fade actions, but vocal chops still need direct manual marker editing, per-slice playback modes, pitch/formant controls, and better metadata than the current fixed eight-pad model.
