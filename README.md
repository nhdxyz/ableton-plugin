...
Current house/UKG workflow focus: cleaner oscillator sources, Osc Warp source motion, Filter Character/Slope modes for bass and stab color, HOME performance macros and club low-end guidance, MOD destination inspection with per-route bypass/delete, SEQ chord/voicing/strum plus one-finger Memory and selective groove transforms, waveform-based sample chopping with click guards and slice styles, section-scoped randomization, and an addable FX rack with throw controls and module presets.
