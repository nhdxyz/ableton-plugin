...
- The HOME `CLUB` assistant now turns this into live patch guidance by flagging root-range issues, low-side stereo risk, Mono spread-lock behavior, Guard/headroom risk, and phase-reset status. The always-visible output meter adds a second quick safety check by labelling the live level as LOW, SAFE, HOT, or CLIP.
...
- The SAMPLE panel now stores per-slice region, pitch, gain, pan, trigger chance, reverse, stutter, stutter repeats, and choke state, with Choke, Pan, and Ghost actions for cutting, placing, and skipping UKG vocal slices.
