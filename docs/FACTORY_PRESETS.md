...
The UKG factory presets include conservative MOD-panel defaults where useful, such as synced LFO cutoff motion on basses, slower Mod Env filter movement on Dred-style bass, and Mod Env/velocity cutoff movement on stabs and plucks. Presets also seed subtle EQ curves for bass weight, low-mid cleanup, vocal chop presence, and stab brightness. Stab, bell, late-hit, and vocal chop presets use light Tremolo/Auto Pan settings for synced motion. Bell, vocal chop, and late-stab presets use restrained Ring Mod for metallic edge.
