...
The current test suite includes `EditorLayoutAudit`, which opens the real plugin editor and checks all panels plus FX detail views for visible controls with empty or out-of-bounds layouts, `PresetSaveAudit`, which verifies nested category saves, normalized preset metadata, and load-by-name behavior, and `RandomCandidateAudit`, which verifies generated-candidate capture, compare, non-destructive cue preview, save-from-slot, recall, and snapshot promotion.
